...
for i in range(1, SIZE+1):
    for j in range(1, SIZE+1):
        Subregion(region=region,
                  lat_start=LAT_START + (SUBREGION_WIDTH*(i-1)),
                  lon_start=LON_START + (SUBREGION_HEIGHT*(j-1)),
                  lat_end=LAT_START+(SUBREGION_WIDTH*i),
                  lon_end=LON_START+(SUBREGION_HEIGHT*j),
                  index=index).save()
        index+=1